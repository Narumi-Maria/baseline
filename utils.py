...
class ImageDataset(Dataset):
    def __init__(self, images_dir, mask_dir, txt_path, img_transform1=None, img_transform2=None, istrain=True,
                 sample_ratio=1, model_name='ResNet18'):
        # with open(txt_path, "r") as f:
        #     reader = csv.reader(f)
        #     reader = list(reader)
        #     reader = reader[1:]
        csv_file = pd.read_csv(txt_path)
        shuffle_csv = csv_file.sample(frac=sample_ratio)
        images_path = list(pd.Series(shuffle_csv["img_path"]))
        labels = list(pd.Series(shuffle_csv["label"]))
        masks_path = list(pd.Series(shuffle_csv["mask_path"]))
        ...
        if self.istrain is True:
            img_list = np.array(images_path)
            np.save(str(len(images_path)) + model_name + "_img_list", img_list)
            print("sample:" + str(len(images_path)))
...
# 加载训练数据
def get_data_loader(transform1, transform2, config, sample_ratio=1, model_name='ResNet18'):
    trainset = ImageDataset(images_dir=config.images_dir, mask_dir=config.mask_dir, txt_path=config.train_txt,
                            ...
                            sample_ratio=sample_ratio, model_name=model_name)
